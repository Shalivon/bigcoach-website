...
└─ about (#about)            — כרטיס ממוזג (2026-07): "אני הבן אדם שיהיה שם כשקשה" + הסיפור של גולן והסמכות בעמודה שנייה. בלי תמונות גלויות — golan.jpg כרקע בשקיפות נמוכה (.ag-bg). העוגן #golan יושב על עמודת גולן בתוך הכרטיס
└─ method (#method)          — השיטה (לפני המסלולים בכוונה: מדריך → תוכנית → הצעה)
└─ programs (#programs)      — סליידשואו נגלל בדסקטופ (sticky + spacer 500vh, scrub לפי scroll, פס התקדמות + נקודות) / כרטיסים במובייל
...
- **תוכניות (דסקטופ, 2026-07)**: סליידשואו נגלל — מסגרת sticky אחת, `show-spacer` 500vh, מיקום הגלילה מחליף פריים (כמו scrubbing). כותרת/תמונה נכנסות ב-fade+scale, פס אדום (`--show-p`) + נקודות. פלוס פותח detail. במובייל: כרטיסים מוערמים, scrub כבוי.
- **פאנלי התוכניות**: `.prog-center` = סטאק ממורכז: כותרת ענקית → `.prog-cap` → `.prog-keep`. כפתור הפלוס בפינה עם העיגול בלבד. במצב open הסטאק נעלם.
